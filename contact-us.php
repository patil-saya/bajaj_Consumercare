...
<meta property="og:url" content="<?php echo $actual_link ?>contact-us.php" />
<meta property="og:image" content="<?php echo $host ?>assets/images/contact-live.jpg" />
   </head>
   <body>
...
$(function() {
  ...
  $('#contactForm').submit(function(){
    ...
    if ((captchaVal !== captchaCode) || (!emailFilter.test(emailText)) || (!phoneFilter.test(phoneText)) || (!nameFilter.test(nameText)) || (messageText < 10)) {
      return false;
    }
   if ((captchaVal !== captchaCode) || (!emailFilter.test(emailText)) || (!phoneFilter.test(phoneText)) || (!nameFilter.test(nameText)) || (messageText < 10)) {
 ...
 $captcha = $_POST['captcha'];
    ...
    if ((empty($_POST['name'])) || (empty($_POST['email'])) || (empty($_POST['contactnumber'])) || (empty($_POST['location'])) || (empty($_POST['message'])) || (empty($_POST['captcha'])))
    {
      echo "<script>sweetAlert('All Fields are required...!');</script>";
        ...
        if($conn){
          $sql = "INSERT INTO form_contactus (name, email, contactnumber, country, message) 
              VALUES (:name, :email, :contactnumber, :country, :message)";
...
//        $message .= '<tr><td><b>Address :</b></td><td>' .$_POST['address'] ."</td></tr>";
        ...
        $header = "From: contact<noreply@bajajconsumercare.com> \r\n";
